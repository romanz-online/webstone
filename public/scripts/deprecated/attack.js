...
function attack() {
  ...
  for (let i = 0; i < numOfChild; i++) {
    document.getElementsByClassName('cardInPlay--player')[i].style.boxShadow =
      '0px 2px 15px 12px #0FCC00'
    document
      .getElementsByClassName('cardInPlay--player')
      [i].classList.add('canAttack')
  }
  ...
  document.querySelectorAll('.cardinplay').forEach(function (e) {
    e.addEventListener('mousedown', function (e) {
      ...
      if (currentAttacker == null) {
        ...
        this.classList.contains('cardInPlay--opponent') ||
        this.id == 'opponentHero'
      ) {
        let currentAttackerElement = document.getElementById(currentAttacker),
          targetElement = document.getElementById(this.id),
          currentAttackerAttack =
            currentAttackerElement.children[0].children[0].innerHTML,
          currentAttackerHealth =
            currentAttackerElement.children[1].children[0].innerHTML,
        ...
        if (currentAttacker == 'playerHeropower') {
          ...
          setTimeout(function () {
            ...
            if (this.id != 'opponentHero') {
              currentAttackerElement.children[1].children[0].style.color =
                '#f20301'
            }
        ...
        if (targetElement.id == 'opponentHero') {
          document.querySelector('#computerdamagevalue').innerText =
            '-' + currentAttackerAttack
        ...
        if (currentAttacker == 'playerHeropower') {
          document
            .getElementById('playerHeropower')
            .classList.remove('canAttack')
        }
          ...
          if (targetHealth <= 0) {
            if (document.getElementById('opponentHeroHealth').innerText <= 0) {
              ...
              if (GAME.opponentBoard.count() == 0) {
                GAME.opponentBoard.htmlElement.style.transform =
                  'translateY(57.5%)'
              }
            }
          }
        ...
        if (
          document
            .getElementById(currentAttacker)
            .classList.contains('cardInPlay--player')
        ) {
...
function gameWon() {
  ...
  setTimeout(function () {
    ...
    $('#game').removeClass('shakeScreenAnim').addClass('shakeScreenAnim')
  }, 750)
  ...
  setTimeout(function () {
    ...
    $('#victory').show()
    $('#victoryImg1, #victoryImg2, #victorylabel').addClass('openMenuAnim')
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        $('#victoryhint').show().addClass('openMenuAnim')
